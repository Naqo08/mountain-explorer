...
def get_ai_facts(mountain_id: int):
    ...
    try: 
        ...
        # Simple post-processing to ensure line breaks
        facts_text = response["output"]
        
        # Replace bullet points that are inline with proper line breaks
        facts_text = facts_text.replace(" • ", "\n• ")
        
        # Ensure it starts with a bullet point
        if not facts_text.strip().startswith("•"):
